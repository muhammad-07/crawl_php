...
if ($result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    ...
    if (!empty($htmlString)) {
      ...
      //PRICE
      $price = "NULL";
      $nodes = $finder->query('*//div[@id="ContentPlaceHolder1_tr_valuepertoken"]//span[contains(@class, "d-block")]//span');
      ...
      if (!is_null($nodes)) {
        ...
        // }
       $price = ltrim($nodes->item(0)->nodeValue, "$"); 
       $price = str_ireplace(",", "", trim($price));
      }

      //Market CAP
      $market_cap = "NULL";
      $nodes = $finder->query('*//div[@id="ContentPlaceHolder1_tr_valuepertoken"]//span[contains(@class, "d-block")]//button');
      
      if (!is_null($nodes)) {
        // foreach($nodes as $node) {
        //  $market_cap = $node->nodeValue;
        // }
        // $market_cap = ltrim($nodes->item(0)->nodeValue, "$"); 
        $market_cap = str_ireplace(",", "", trim($nodes->item(0)->nodeValue));
        $market_cap = str_ireplace("$", "", trim($market_cap));
      }

      // Symbol
      $symbol = "NULL";
      $nodes = $finder->query('*//main[@id="content"]//*[contains(@class, "container")]//div[contains(@class, "media-body")]//span');
      // print_r($nodes);
      if (!is_null($nodes)) {
        foreach($nodes as $node) {
      ...
      if (!is_null($nodes)) {
        foreach ($nodes as $k => $element) {
          ...
          if ($k == 0)
            $links["email"] = "'" . str_ireplace('Email: ', "", $element->getAttribute('data-original-title')) . "'";
          else {
            $crawledLinks[] = $element->getAttribute('href');
            if (strpos($element->getAttribute('href'), "t.me") !== false || strpos($element->getAttribute('href'), "telegram") !== false) {
